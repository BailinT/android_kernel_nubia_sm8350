...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if (dsi_display->panel->hbm_mode) {
		NUBIA_DEBUG("Panel in HBM mode, not allowed set brightness!!\n");
		return 0;
	}

	/* HBM mode is 0xFFF(4095), Normal mode max is 0x7FF(2047) */
	if ((brightness > dsi_display->panel->bl_config.brightness_max_level) && (brightness != 4095))
		brightness = dsi_display->panel->bl_config.brightness_max_level;
	if ((brightness > c_conn->thermal_max_brightness) && (brightness != 4095))
		brightness = c_conn->thermal_max_brightness;

	if (brightness != 4095)
		bl_lvl = nubia_backlight_covert(dsi_display,brightness);
	else
		bl_lvl = brightness;
#else
